...
const ProductCard = ({ product }) => {
    ...
    const { countryInfo, getPriceForCountry, formatPrice } = useCountryPricing();

    // Select variant for product card display:
    // 1. First priority: variant that has a country override matching the user's detected country
    // 2. Second priority: first variant in product.colorVariants array
    const activeVariant = React.useMemo(() => {
        if (!product || !Array.isArray(product.colorVariants) || product.colorVariants.length === 0) {
            return null;
        }
        const userCode = countryInfo?.countryCode?.toUpperCase();
        if (userCode) {
            const overrideVariant = product.colorVariants.find(v =>
                Array.isArray(v.countryPricing) &&
                v.countryPricing.some(cp => cp.countryCode?.toUpperCase() === userCode)
            );
    ...
    // Get country-aware pricing (variant override > product override > auto-converted INR)
    const pricing = getPriceForCountry(product, activeVariant);

    // Image resolution: variant image first, then product images, then placeholder
    const cardImage = activeVariant?.images && activeVariant.images.length > 0
        ? activeVariant.images[0]
        : (product.images && product.images.length > 0 ? product.images[0] : (product.image || "https://via.placeholder.com/300"));
            ...
            <div className="p-3 md:p-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto">
                    ...
                    <button
                        ...
                        className={`w-full py-2 md:py-2.5 rounded-lg md:rounded-xl transition-all flex items-center justify-center gap-2 text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-lg ${isOutOfStock ? 'bg-gray-200 text-gray-400 cursor-not-allowed shadow-none' : 'bg-primary text-white hover:bg-orange-600 shadow-orange-500/10'}`}
                    >
