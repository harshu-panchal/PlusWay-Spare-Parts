...
const HomeSectionManagement = () => {
  ...
  useEffect(() => {
    if (isModalOpen) {
      fetchOptionsForType(formData.displayType);
    }
  }, [isModalOpen, formData.displayType]);

  const fetchSections = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(API_ENDPOINTS.ADMIN_HOME_SECTIONS, {
  ...
  const fetchOptionsForType = async (displayType, force = false) => {
    if (!force && optionCache[displayType]?.length > 0) {
      return;
    }
    ...
    try {
      ...
      switch (displayType) {
        case "brands":
          url = `${API_ENDPOINTS.BRANDS}?all=true`;
          listKey = "brands";
          headers = undefined;
          break;
        case "models":
          url = `${API_ENDPOINTS.MODELS}?all=true`;
          listKey = "models";
          headers = undefined;
          break;
        case "products":
          url = `${API_ENDPOINTS.ADMIN_PRODUCTS}?pageNumber=1&pageSize=1000`;
          listKey = "products";
          headers = getAuthHeaders();
          break;
        case "categories":
        default:
          url = `${API_ENDPOINTS.CUSTOMER_CATEGORIES}?all=true`;
          listKey = "categories";
          headers = undefined;
          break;
      }
      ...
      const items = normalizeList(data, listKey);
      ...
      console.error(`Error fetching ${displayType}:`, error);
      const isNetworkError =
        error.message === "Network Error" || error.code === "ERR_NETWORK";
      const message = isNetworkError
        ? `Cannot reach the API server. Start the backend (port 5001) and restart the frontend dev server.`
        : error.response?.data?.message ||
          error.message ||
          `Failed to load ${displayType}. Please try again.`;
  ...
  const retryFetchOptions = () => {
    ...
    fetchOptionsForType(formData.displayType, true);
  };

  const getItemsForDisplayType = (displayType) => {
    return optionCache[displayType] || [];
  };

  const getItemLabel = (item, displayType) => {
    switch (displayType) {
      case "brands":
        return item.name;
      case "models":
        return item.brand?.name ? `${item.name} (${item.brand.name})` : item.name;
      case "products":
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSection) {
        ...
        await axios.post(API_ENDPOINTS.ADMIN_HOME_SECTIONS, formData, config);
      }
  ...
  const toggleItem = (itemId) => {
    setFormData((prev) => {
      if (prev.categories.includes(itemId)) {
        return {
          ...prev,
          categories: prev.categories.filter((id) => id !== itemId),
        };
      }
      return { ...prev, categories: [...prev.categories, itemId] };
    });
  };
  ...
  const currentItems = getItemsForDisplayType(formData.displayType);
  const filteredItems = currentItems.filter((item) => {
    if (!optionSearchTerm.trim()) return true;
    const label = getItemLabel(item, formData.displayType).toLowerCase();
    return label.includes(optionSearchTerm.trim().toLowerCase());
  });
  const selectLabel =
    DISPLAY_TYPES.find((type) => type.value === formData.displayType)?.label ||
    "Items";
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold text-gray-600 text-sm">Display Type</th>
            ...
            {sections.map((section) => (
              <tr key={section._id} className="hover:bg-gray-50/50">
                ...
                <td className="px-6 py-4">
                  <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-100 capitalize">
                    {section.displayType || "categories"}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {section.categories.map((item) => (
                      <span
                        ...
                        {getItemLabel(item, section.displayType || "categories")}
                      </span>
                    ))}
                  </div>
                </td>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          productsPerRow: parseInt(e.target.value, 10) || 4,
                        })
                      }
                      className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    />
              ...
              <div>
                ...
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    placeholder={`Search ${selectLabel.toLowerCase()}...`}
                ...
                <div className="border border-gray-200 rounded-lg max-h-60 overflow-y-auto p-4 bg-gray-50">
                  {optionsLoading ? (
                    <p className="text-sm text-gray-500 text-center py-6">
                      Loading {selectLabel.toLowerCase()}...
                    </p>
                  ) : optionsError ? (
                    ...
                    <p className="text-sm text-gray-500 text-center py-6">
                      No {selectLabel.toLowerCase()} found.
                    </p>
                  ) : filteredItems.length === 0 ? (
                    <p className="text-sm text-gray-500 text-center py-6">
                      No {selectLabel.toLowerCase()} match your search.
                    </p>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {filteredItems.map((item) => (
                        <label
                          key={item._id}
                          className={`flex items-center gap-2 p-2 rounded border cursor-pointer transition-all ${formData.categories.includes(item._id) ? "bg-blue-50 border-blue-200" : "bg-white border-gray-100 hover:border-gray-300"}`}>
                          <input
                            type="checkbox"
                          ...
                          <span
                            ...
                            title={getItemLabel(item, formData.displayType)}>
                            {getItemLabel(item, formData.displayType)}
                          </span>
                        </label>
                      ))}
                    </div>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Selected: {formData.categories.length} {selectLabel.toLowerCase()}
                </p>
              </div>
