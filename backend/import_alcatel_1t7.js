...
async function uploadImage(filePath) {
  try {
    const result = await cloudinary.v2.uploader.upload(filePath, {
      folder: 'plusway_spare_parts/ALCATEL_1T7',
    });
...
async function getOrCreateCategoryFromTitle(title) {
  ...
  } else if (lowerTitle.includes('flip cover') || lowerTitle.includes('tempered glass')) {
...
async function importData() {
  ...
  try {
    ...
    let model = await Model.findOne({ name: 'Alcatel 1T 7' });
    if (!model) {
      console.log("Creating Model 'Alcatel 1T 7'...");
      model = await Model.create({
        name: '1T 7',
        brand: brand._id,
        released: '2018',
        displaySize: '7.00 inches',
      });
    }

    const scrapedBaseDir = path.resolve(__dirname, '../scraped_data/ALCATEL_1T7');
    ...
    console.log(`Found ${productFolders.length} products to import for Alcatel 1T 7.`);
    ...
    for (const folder of productFolders) {
      ...
      const description =
        details.description || `High quality ${details.title} spare part for Alcatel 1T 7.`;
      ...
      const code = `ALCT7-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
    ...
    console.log('Alcatel 1T 7 import completed successfully!');
    process.exit(0);
  } catch (error) {
    console.error('Alcatel 1T 7 import failed:', error);
    process.exit(1);
  }
}
