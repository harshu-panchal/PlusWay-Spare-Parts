...
async function uploadImage(filePath) {
    try {
        const result = await cloudinary.v2.uploader.upload(filePath, {
            folder: "plusway_spare_parts/ACERONE_S272E4",
        });
...
async function importData() {
    ...
    try {
        ...
        // Note: Using "ACERONE LIQUID S272E4" as name. 
        // If frontend expects "Acerone Liquid S272E4", we should arguably be consistent with previous fix?
        // The previous fix moved products to "Acerone Liquid S162E4" (finding by ID). 
        // Here we are creating a NEW model. I will use Title Case to match typical seed data if possible,
        // or just "ACERONE LIQUID S272E4" as per user request.
        // User asked for: 'ACERONE LIQUID S272E4'
        let model = await Model.findOne({ name: 'Acer ACERONE LIQUID S272E4' });
        if (!model) {
            // Also check for Title Case just in case
            model = await Model.findOne({ name: 'Acer Acerone Liquid S272E4' });
        }

        if (!model) {
            console.log("Creating Model 'Acer ACERONE LIQUID S272E4'...");
            model = await Model.create({
                name: 'Acer ACERONE LIQUID S272E4',
                brand: brand._id,
                released: '2023',
                displaySize: 'Unknown'
            });
        ...
        const scrapedBaseDir = path.resolve(__dirname, '../../scraped_data/ACERONE_LIQUID_S272E4');
