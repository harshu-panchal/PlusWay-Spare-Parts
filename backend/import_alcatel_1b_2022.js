...
async function uploadImage(filePath) {
    try {
        const result = await cloudinary.v2.uploader.upload(filePath, {
            folder: "plusway_spare_parts/ALCATEL_1B_2022",
        });
...
async function importData() {
    ...
    try {
        ...
        let model = await Model.findOne({ name: 'Alcatel 1B 2022' });
        if (!model) {
            console.log("Creating Model 'Alcatel 1B 2022'...");
            model = await Model.create({
                name: '1B 2022', // The pre-save hook will make it 'Alcatel 1B 2022'
                brand: brand._id,
                released: 'May 2022',
                displaySize: '5.50 inches'
            });
        }
        ...
        const scrapedBaseDir = path.resolve(__dirname, '../../scraped_data/ALCATEL_1B_2022');
        ...
        for (const folder of productFolders) {
            ...
            const code = `ALCB-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
