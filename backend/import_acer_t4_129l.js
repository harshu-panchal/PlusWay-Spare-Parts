...
const SCRAPED_DIR = path.resolve(__dirname, '../../scraped_data/ONE_10_T4-129L');
const BRAND_NAME = 'Acer';
const MODEL_NAME = 'One 10 T4-129L';
    ...
    try {
        ...
        for (const folder of productFolders) {
            ...
            for (const imgFile of imagesToUpload) {
                const url = await uploadToCloudinary(path.join(folderPath, imgFile), `plusway_spare_parts/ACER_T4_129L`);
                if (url) imageUrls.push(url);
            }

            // Parse Price
            // Price format "1,299.00 Rs." or "Rs. 1,299.00"
            let price = 0;
            if (details.price) {
