...
const SOURCE_URL =
  'https://www.maxbhi.com/search.html?search_performed=Y&pname=Y&pkeywords=Y&q=alcatel+1t+7&items_per_page=128';
const OUTPUT_DIR = path.resolve(__dirname, '../scraped_data/ALCATEL_1T7');
...
async function buildScrapedData() {
  ...
  // Maxbhi search listing: each product in grid/list with link, image, title, prices
  $('.ty-column4, .ty-grid-list__item').each((_, el) => {
    const container = $(el);
    ...
    const mrpText =
      container.find('.ty-price-num span.ty-price-num').first().text().trim() ||
      container.find('.ty-price-num').first().text().trim();
    const priceText =
      container.find('.ty-price_actual .ty-price-num').first().text().trim() ||
      container.find('.ty-price').first().text().trim();
  ...
  console.log(`Found ${products.length} products in Alcatel 1T 7 search results.`);
  ...
  for (const product of products) {
    ...
    // Fetch product detail page for full specs
      ...
      if (product.href) {
        const detailHtml = await fetchHtml(
          product.href.startsWith('http')
            ? product.href
            : `https://www.maxbhi.com/${product.href.replace(/^\//, '')}`
        );
  ...
  console.log(`Alcatel 1T 7 scraped data prepared in: ${OUTPUT_DIR}`);
}

buildScrapedData().catch((err) => {
  console.error('Failed to build Alcatel 1T 7 scraped data:', err);
  process.exit(1);
});
